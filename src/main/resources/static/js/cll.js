app.controller('cllController',
    ...
    $(function () {
        showModal($rootScope);
        $compile($('#patientBody'))($scope);  //Recompile the charged template, otherwise doesn't read angularjs variables
    ...
    // If it's not myeloproliferative
    if ($scope.medicalRecord.category !== 4){
    ...
    let timelines =$('#timelineLlc li').toArray();
    ...
    $scope.sectionClicked = function($event){
        // $($event.currentTarget).index()   putting li as ng-click
        // $($event.target.parentNode.parentNode).index()  putting h4 as ng-click
    ...
    $scope.addMyeloproliferativeDiagnosis = function(){
        ...
        $http({
            ...
            fillModal($rootScope, "Result", "Myeloproliferative diagnosis successfully registered", "alert-success", "btn-success", true);
        ...
        for (let ind=0; ind<$scope.myeloTherapy.length; ind++) {
            ...
            $http({
                ...
                if (nextPhasePass === $scope.myeloTherapy.length) {
                    fillModal($rootScope, "Result", "Therapy myeloproliferative registered successfully", "alert-success", "btn-success", true);
                    nextPhase($scope.currentSection++, true);
                }
            }, function (err) {
                console.log("Error registering myeloproliferative dyagnosis");
                console.log(err);
                fillModal($rootScope, "Error", err.data.message, "alert-danger", "btn-danger", true);
            });
        }
